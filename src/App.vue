<template >
  <div v-if="!isNotShowHeaderAndSidebar"  class="flex flex-col -z-10" @click="handleCloseModal">
    ...
    <div class="bg-dashboardBg flex">
      <div class="lg:w-[8vw] lg:flex lg:items-center items-start flex-col toggle-sidenav" :class="[isToggle ? 'block' : 'hidden']">
        <span
        ...
        <div class="border-r-2 border-sidebarBorder h-full flex flex-col lg:items-center items-start lg:w-full w-[30vw] bg-white-mobile">
          <SidebarIcon v-for="route in routes" :route-path="route.path" :key="route.id" :title="route.title"
            :icon="route.icon" classes="mb-2" @click="handleSetActive(route.id,route.title)" :isActive="activeId === route.id" />
        </div>
      </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    isAccountModule(){
      ...
      if(hrefPath=='/otp' || hrefPath == '/forgot-password' || hrefPath == '/signup' || hrefPath == '/'){
