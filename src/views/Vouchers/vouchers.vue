    ...
    <div class="bg-white w-full min-h-[25rem] rounded-[0.938rem] py-3  md:px-6 px-3 shadow overflow-auto">
      ...
      <SmeTable v-if="paginatedTableData.length" :tableName="tableName" :tableHeaderData="routerTableHeaders">
        <template v-slot:tableBodyData>
          <div v-for="item in filteredTableData" :key="item.id"
...
export default defineComponent({
  ...
  methods: {
    ...
    getVouchersData() {
      instance.get(`vouchers/?realm__sme=1`)
  ...
  computed: {
    ...
    filteredTableData(): any[] {
      ...
      const filteredData = this.voucherList.filter(item => {
        ...
        return time_valid.includes(query) || value.includes(query) || source.includes(query) || created.includes(query);
      });
